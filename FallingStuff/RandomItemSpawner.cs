...
//
////Add DDA for fall speed among level
//
...
{
    public string folderPath = "Prefabs/"; // Path folder dalam Resources tanpa "Assets/Resources"
    public float spawnHeight = 5.5f; // Ketinggian spawn untuk objek yang jatuh
    public float minSpawnInterval = 2f; // Interval waktu spawn minimum
    public float maxSpawnInterval = 3f; // Interval waktu spawn maksimum
    public Vector3 itemScale = new Vector3(0.5f, 0.5f, 0.5f); // Faktor skala yang ingin diterapkan pada objek yang jatuh
    ...
    // New customizable parameters for fall speed and gravity
    [Header("Item Fall Settings")]
    [Range(0, 10)] public float gravityScale = 0.01f;  // Skala gravitasi yang bisa disesuaikan di Inspector
    [Range(0f, 10f)] public float minFallSpeed = 0.21f;  // Kecepatan jatuh minimum
    [Range(0f, 11f)] public float maxFallSpeed = 3f;  // Kecepatan jatuh maksimum
    [Range(1, 5)] public float minSpawnAmount = 1f;  // Jumlah item minimum yang akan muncul secara bersamaan
    [Range(1, 5)] public float maxSpawnAmount = 3f;  // Jumlah item maksimum yang akan muncul secara bersamaan
    ...
    IEnumerator SpawnItemsRandomly()
    {
        while (true)
        {
            ...
            for (int i = 0; i < itemsToSpawn; i++)
            {
                ...
                // Atur properti Rigidbody2D untuk memastikan item jatuh dengan kecepatan yang tidak terlalu cepat
                UpdateFallSettingsBasedOnLevel(rb); // Update kecepatan jatuh dan gravitasi berdasarkan level
    ...
    // Fungsi untuk menyesuaikan kecepatan jatuh dan gravitasi berdasarkan level
    private void UpdateFallSettingsBasedOnLevel(Rigidbody2D rb)
    {
        if (xpLevelManager != null)
        {
            int level = xpLevelManager.GetCurrentLevel();

            // // Menyesuaikan nilai-nilai fall speed dan gravity
            // gravityScale = Mathf.Clamp(2f + level * 0.2f, 2f, 5f); // Skala gravitasi meningkat seiring level, namun dibatasi antara 2 dan 5
            // minFallSpeed = Mathf.Clamp(3f + level * 0.5f, 3f, 10f); // Kecepatan jatuh minimum meningkat
            // maxFallSpeed = Mathf.Clamp(8f + level * 0.5f, 8f, 15f); // Kecepatan jatuh maksimum meningkat
            
            // Mengatur nilai dengan kenaikan yang seimbang tanpa batasan
            gravityScale = 0.01f + level * 0.02f; // Skala gravitasi bertambah secara seimbang
            minFallSpeed = 0.21f + level * 0.05f; // Kecepatan jatuh minimum bertambah
            maxFallSpeed = 0.3f + level * 0.06f; // Kecepatan jatuh maksimum bertambah

            // Terapkan perubahan ke Rigidbody2D
            rb.gravityScale = gravityScale;
            Debug.Log("Min Fall Speed: " + minFallSpeed + ", Max Fall Speed: " + maxFallSpeed);
        }
    }
}
